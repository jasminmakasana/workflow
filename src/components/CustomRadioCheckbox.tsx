...
export class CustomRadioCheckbox extends Component<any, any> {
  ...
  displayOptions = (optionData: any) => {
    ...
    Object.keys(optionData).map((key, i) => {
      retData.push(
        <div
          key={`${key}-${id}-${optionData[i].value}`}
          className="d-inline-block p-r-20"
        >
          <input
            type={type}
            name={name}
            id={`${id}-${optionData[i].value}`}
            checked={value.indexOf(optionData[i].value) !== -1}
            className={`${inputClass} ${isValid ? "" : "is-invalid error"}`}
            onChange={this.onChange}
            value={optionData[i].value}
          />
          <label
            htmlFor={`${id}-${optionData[i].value}`}
            className="d-inline-block w-auto p-l-5 font-w-normal"
          >
            {optionData[i].label}
          </label>
        </div>
